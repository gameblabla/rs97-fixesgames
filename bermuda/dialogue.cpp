...
#ifdef OPENDINGUX
static const Rect _dialogueBackgroundRect = { 0, 0, 640, 480 };
#else
...
#ifdef OPENDINGUX
	for (int i = 0; i < 32; ++i) {
		uint16 chr = fontData[offset + (i>>1)];
		for (int b = 0; b < 32; ++b) {
			if (chr & (1 << (b>>1))) {
#else
	for (int i = 0; i < 16; ++i) {
...
#ifdef OPENDINGUX
			if (stringLen > _dialogueTextRect.w/2) {
#else
	...
	for (int i = 0; i < _dialogueChoiceCounter; ++i) {
		...
		for (char *p = _dialogueChoiceText[i]; *p; ++p) {
			if (p == substringOffset[i][substring]) {
				...
				int chr = (uint8)*p;
				if (y + yPerLine >= _dialogueTextRect.h) {
					break;
				}
#ifdef OPENDINGUX
				drawChar(textBuffer + (_dialogueTextRect.h - 1 - y) * _dialogueTextRect.w + x*2, _dialogueTextRect.w, _fontData, chr, choiceColor);
#else
