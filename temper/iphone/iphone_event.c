...
// When you press a button on iphone, which config option should it
// correspond to? That is, which in config.pad, the order that
// the iphone buttons are given in the menu. This says what, for each
// bit.

u32 iphone_to_config_map[] =
{
  GP2X_CONF_UP,           // 0
  GP2X_CONF_NONE,         // 1
  GP2X_CONF_LEFT,         // 2
...
void update_input()
{
  u32 iphone_buttons = iphone_joystick_read();
  u32 button_status = 0;
  static u32 last_iphone_buttons;

  u32 i;

  // vol up + vol down cancels both but presses the imaginary vol mid button
  if((iphone_buttons & GP2X_VOL_UP) && (iphone_buttons & GP2X_VOL_DOWN))
  {
    iphone_buttons &= ~(GP2X_VOL_UP | GP2X_VOL_DOWN);
    iphone_buttons |= GP2X_VOL_MID;
  }

  for(i = 0; i < 28; i++)
  {
    if(iphone_buttons & (1 << i))
    {
      // Now perform the action and update the button status
      button_status |= button_action(iphone_to_config_map[i],
       last_iphone_buttons & (1 << i));
    }
  }

  last_iphone_buttons = iphone_buttons;
...
gui_action_type get_gui_input()
{
  ...
  u32 buttons = iphone_joystick_read();
  u32 new_buttons;
