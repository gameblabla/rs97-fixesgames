...
extern RGB16 np2_pal_filter;

static inline RGB16 mixpal(RGB16 a,RGB16 b)
{
	return ((a & np2_pal_filter)>>1) + ((b & np2_pal_filter)>>1);
}
...
const UINT8	*p;
	...
	do {
		if (*(UINT16 *)(sdraw->dirty + y)) {
			for (x=0; x<sdraw->width; x+=2) {
				work = mixpal( mixpal(np2_pal16[p[x+0] + NP2PAL_GRPH],
						np2_pal16[p[x+1] + NP2PAL_GRPH]),
					   mixpal(np2_pal16[p[x+0+SURFACE_WIDTH] + NP2PAL_GRPH],
							  np2_pal16[p[x+1+SURFACE_WIDTH] + NP2PAL_GRPH]));
				
				*(UINT16 *)q = (UINT16)(work);
				q += sdraw->xalign;
			}
			q -= xbytes;
		}
...
const UINT8	*q;
	...
	do {
		if (*(UINT16 *)(sdraw->dirty + y)) {
			for (x=0; x<sdraw->width; x+=2) {
				work = mixpal(
							  mixpal(np2_pal16[p[x+0] + q[x+0] + NP2PAL_GRPH],
							  np2_pal16[p[x+1] + q[x+1] + NP2PAL_GRPH])
							,
						mixpal(np2_pal16[p[x+0+SURFACE_WIDTH] +
										q[x+0+SURFACE_WIDTH] + NP2PAL_GRPH],
						       np2_pal16[p[x+1+SURFACE_WIDTH] + 
										 q[x+1+SURFACE_WIDTH] + NP2PAL_GRPH]));
				
				*(UINT16 *)r = (UINT16)(work);
...
const UINT8	*p;
	...
	do {
		if (sdraw->dirty[y]) {
			for (x=0; x<sdraw->width; x+=2) {
				work = mixpal( np2_pal16[p[x+0] + NP2PAL_GRPH]
							  ,np2_pal16[p[x+1] + NP2PAL_GRPH]);
...
const UINT8	*q;
	...
	do {
		if (sdraw->dirty[y]) {
			for (x=0; x<sdraw->width; x+=2) {
				work = mixpal(
							  np2_pal16[p[x+0] + q[x+0] + NP2PAL_GRPH],
							  np2_pal16[p[x+1] + q[x+1] + NP2PAL_GRPH]
							  );
