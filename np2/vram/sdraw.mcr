...
const UINT8	*p;
	...
	do {
		if (sdraw->dirty[y]) {
			for (x=0; x<sdraw->width; x++) {
#ifdef PSP
				SDSETPIXEL(x, q, p[x] + NP2PAL_GRPH);
#else
...
const UINT8	*q;
	...
	do {
		if (sdraw->dirty[y]) {
			for (x=0; x<sdraw->width; x++) {
#ifdef PSP
				SDSETPIXEL(x, r, p[x] + q[x] + NP2PAL_GRPH);
#else
...
const UINT8	*p;
	...
	do {
		...
		if (sdraw->dirty[y+1]) {
			for (x=0; x<sdraw->width; x++) {
#ifdef PSP
				SDSETPIXEL(x, q, (p[x] >> 4) + NP2PAL_TEXT);
#else
...
const UINT8	*q;
	...
	do {
		...
		if (sdraw->dirty[y+1]) {
			for (x=0; x<sdraw->width; x++) {
#ifdef PSP
				SDSETPIXEL(x, r, (q[x] >> 4) + NP2PAL_TEXT);
#else
...
const UINT8	*p;
	...
	do {
		...
		if (sdraw->dirty[y+1]) {
			for (x=0; x<sdraw->width; x++) {
#ifdef PSP
				SDSETPIXEL(x, q, p[x] + NP2PAL_SKIP);
#else
