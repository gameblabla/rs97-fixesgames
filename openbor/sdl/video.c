...
int video_copy_screen(s_screen* src)
{
	...
	if(src->pixelformat == 4){ // 32bit
		...
		uint32_t x, y;
		SDL_Color col;
		tmp = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 240, 16, 0, 0, 0, 0);
		uint8_t *s = (uint8_t*)src->data;
		uint16_t *d = (uint16_t*)tmp->pixels;
		uint32_t *s2 = (uint32_t*)tmp->pixels;
		uint32_t *d2 = (uint32_t*)screen->pixels;
		for(y=0; y<240; y++, s2 += 160, d2 += 320)
		{
			for(x=0; x<320; x++)
			{
				col = colors[*s++];
