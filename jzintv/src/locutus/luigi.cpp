...
void t_luigi_block::encode( t_byte_vec& enc_data ) const
{
    ...
    // XXX: if scrambling, this is where we could insert the scrambler
...
class t_luigi_mem_map_table
{
    private:
        ...
        void reset()
        {
            ...
            memset( (void *)pfl_map, 0, sizeof( pfl_map ) );
...
void t_luigi::deserialize( t_locutus &locutus, const t_byte_vec &data )
{
    ...
    // -------------------------------------------------------------------- //
    //  Check magic number.  Currently only support version 0 files.        //
    // -------------------------------------------------------------------- //
    if ( data[0] != 'L' || data[1] != 'T' || data[2] != 'O' || data[3] != 0 )
        throw string("t_luigi:  invalid magic number");

    // -------------------------------------------------------------------- //
    //  Check header checksum                                               //
    // -------------------------------------------------------------------- //
    uint8_t      crc_header_stored   = data[15];
    ...
    for (int i = 0; i < 8; i++)
        ...
        {
            int  feat_num = i * 8 + j;
            bool feat_val = (data[i + 4] >> j) & 1;

            locutus.set_feature_flag( feat_num, feat_val );
        }

    t_byte_vec::const_iterator data_i = data.begin() + 16;
    ...
    while (data_i != data_e)
    {
        ...
        t_luigi_block block( data_i, data_e );      // advances data_i

        // Apply it to Locutus
        switch ( block.get_type() )
        {
            case LUIGI_BLKTYPE_SCRAMBLE:    break;  // unsupported, ignore
...
t_byte_vec t_luigi::serialize( const t_locutus& locutus )
{
    ...
    uint64_t flags = 0;
    for (int i = 0; i < 64; i++)
        flags |= uint64_t( locutus.get_feature_flag( i ) ) << i;

    enc_data.push_back( (flags >>  0) & 0xFF );
    enc_data.push_back( (flags >>  8) & 0xFF );
    enc_data.push_back( (flags >> 16) & 0xFF );
    enc_data.push_back( (flags >> 24) & 0xFF );
    enc_data.push_back( (flags >> 32) & 0xFF );
    enc_data.push_back( (flags >> 40) & 0xFF );
    enc_data.push_back( (flags >> 48) & 0xFF );
    enc_data.push_back( (flags >> 56) & 0xFF );
