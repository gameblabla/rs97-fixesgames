...
class t_loc_to_bin_impl
{
  ...
  public:
    ...
    bool process()
    {
        ...
        sort( preloads.begin(),     preloads.end()      );
        ...
        for ( int page = 0; page < 0x10; ++page )
        {
            ...
            if ( !perm[ LOCUTUS_PERM_BANKSW ] ) 
            {
                ...
                sprintf(buf, 
                    ...
                    chap, page, (unsigned int)perm.to_ulong());
...
void t_loc_to_bin_impl::write_mappings() 
{
    ...
    fprintf( f_cfg, "[mapping]\n" );

    t_hunk_vec::const_iterator mappings_i = mappings.begin();

    while ( mappings_i != mappings.end() )
    {
        const uint32_t len = mappings_i->data.size();

        fprintf( f_cfg, "$%.4X - $%.4X = $%.4X",
                 file_offset, file_offset + len - 1, mappings_i->s_addr );

        if ( mappings_i->page == PAGE_NONE )
            fputc( '\n', f_cfg );
        else
            fprintf( f_cfg, " PAGE %1X\n", mappings_i->page );


        fwrite( (const void *)&(mappings_i->data[0]), 2, len, f_bin );

        file_offset += mappings_i->data.size();
...
void t_loc_to_bin_impl::write_preloads() 
{
    ...
    while ( preloads_i != preloads.end() )
    {
        ...
        fwrite( (const void *)&(preloads_i->data[0]), 2, len, f_bin );
