 ...
 *  Author:   J. Zbiciak
 *  $Id: ivoice.c,v 1.1 2000/09/28 00:05:49 im14u2c Exp $
 * ============================================================================
 ...
 *  a 1-deep FIFO, and so LRQ with go to 1 before the SP0256 is finished
 *  speaking.
 *
...
static const char rcs_id[]="$Id: ivoice.c,v 1.1 2000/09/28 00:05:49 im14u2c Exp $";
...
/* ======================================================================== */
static INLINE sint_16 limit (sint_16 s);
static INLINE uint_32 bitrev(uint_32 val);
static int            lpc12_update(lpc12_t *f, int, sint_16 *, uint_32 *);
static void           lpc12_regdec(lpc12_t *f);
static uint_32        sp0256_getb(ivoice_t *ivoice, int len);
...
/* ======================================================================== */
static int lpc12_update(lpc12_t *f, int num_samp, sint_16 *out, uint_32 *optr)
{
    int i, j;
    sint_16 samp;
    int do_int;
...
static const uint_16 sp0256_datafmt[] =
{
    /* -------------------------------------------------------------------- */
    /*  OPCODE 1111: PAUSE                                                  */
    /* -------------------------------------------------------------------- */
...
static const sint_16 sp0256_df_idx[16 * 8] = 
{
...
/* ======================================================================== */
static int ivoice_rdrom(ivoice_t *iv, int page)
{
    uint_8 *rom;
    char buf[32];
    FILE *f;
    ...
    printf("ivoice: added %s at SP0256 address $%.4X.0\n", buf, page << 12);
...
uint_32 ivoice_tk(periph_t *per, uint_32 len)
{
    ...
    while (ivoice->sound_current < until)
    {
        ...
        if (samples > 0) do 
        {
            ...
            /* ------------------------------------------------------------ */
            /*  If our repeat count expired, emulate the microcontroller.   */
            /* ------------------------------------------------------------ */
            if (ivoice->filt.rpt <= 0)
                sp0256_micro(ivoice);
            ...
            samples -= did_samp;
        } while (ivoice->filt.rpt >= 0 && samples > 0);
...
void ivoice_wr(periph_t *per, periph_t *ign, uint_32 addr, uint_32 data)
{
    ...
    if (addr == 0)
    {
        ...
        /* ---------------------------------------------------------------- */
        /*  Set LRQ to 1 and load the 8 LSBs of the data into the ALR reg.  */
        /*  We take the command address, and multiply by 2 bytes to get     */
        /*  the new PC address.                                             */
        /* ---------------------------------------------------------------- */
        ivoice->lrq = 0;
...
)
{
    ...
    if (wind == -1)
    {
        if (rate > 10000)   wind = rate / 5000 + 1;
        else                wind = 20000 / rate + 1;

        if (wind < 1) wind = 1;

        printf("ivoice:  Automatic sliding-window setting: %d\n", wind);
    }
    ...
    ivoice->periph.min_tick  = snd_buf;
    ivoice->periph.max_tick  = snd_buf * 5;
    ...
    ivoice->filt.rng = 1;
    ivoice->wind     = wind;
    ivoice->window   = calloc(wind, sizeof(int));
    ivoice->wind_sum = 0;
    if (!ivoice->window)
    {
    ...
    /* -------------------------------------------------------------------- */
    ivoice->scratch = calloc(SCBUF_SIZE, sizeof(sint_16));
