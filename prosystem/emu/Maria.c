...
// StoreCell
// ----------------------------------------------------------------------------
static inline void _maria_StoreCell(byte data) {
...
static inline void maria_WriteLineRAM(word* buffer) {
  ...
  if(rmode == 0) {
    ...
    for(index = 0; index < MARIA_LINERAM_SIZE; index += 4) {
      *pix++ = maria_GetColor((maria_lineRAM[index + 0] & 16) | ((maria_lineRAM[index + 0] & 8) >> 3) | ((maria_lineRAM[index + 0] & 2)));
      *pix++ = maria_GetColor((maria_lineRAM[index + 0] & 16) | ((maria_lineRAM[index + 0] & 4) >> 2) | ((maria_lineRAM[index + 0] & 1) << 1));
      *pix++ = maria_GetColor((maria_lineRAM[index + 1] & 16) | ((maria_lineRAM[index + 1] & 8) >> 3) | ((maria_lineRAM[index + 1] & 2)));
      *pix++ = maria_GetColor((maria_lineRAM[index + 1] & 16) | ((maria_lineRAM[index + 1] & 4) >> 2) | ((maria_lineRAM[index + 1] & 1) << 1));
      *pix++ = maria_GetColor((maria_lineRAM[index + 2] & 16) | ((maria_lineRAM[index + 2] & 8) >> 3) | ((maria_lineRAM[index + 2] & 2)));
      *pix++ = maria_GetColor((maria_lineRAM[index + 2] & 16) | ((maria_lineRAM[index + 2] & 4) >> 2) | ((maria_lineRAM[index + 2] & 1) << 1));
      *pix++ = maria_GetColor((maria_lineRAM[index + 3] & 16) | ((maria_lineRAM[index + 3] & 8) >> 3) | ((maria_lineRAM[index + 3] & 2)));
      *pix++ = maria_GetColor((maria_lineRAM[index + 3] & 16) | ((maria_lineRAM[index + 3] & 4) >> 2) | ((maria_lineRAM[index + 3] & 1) << 1));
    }
  }
  else if(rmode == 3) {
    for(index = 0; index < MARIA_LINERAM_SIZE; index += 4) {
      *pix++ = maria_GetColor((maria_lineRAM[index + 0] & 30));
      *pix++ = maria_GetColor((maria_lineRAM[index + 0] & 28) | ((maria_lineRAM[index + 0] & 1) << 1));
      *pix++ = maria_GetColor((maria_lineRAM[index + 1] & 30));
      *pix++ = maria_GetColor((maria_lineRAM[index + 1] & 28) | ((maria_lineRAM[index + 1] & 1) << 1));
      *pix++ = maria_GetColor((maria_lineRAM[index + 2] & 30));
      *pix++ = maria_GetColor((maria_lineRAM[index + 2] & 28) | ((maria_lineRAM[index + 2] & 1) << 1));
      *pix++ = maria_GetColor((maria_lineRAM[index + 3] & 30));
      *pix++ = maria_GetColor((maria_lineRAM[index + 3] & 28) | ((maria_lineRAM[index + 3] & 1) << 1));
    }
  }
}
...
uint maria_RenderScanline( ) {
  ...
  if((memory_ram[CTRL] & 96) == 64 && maria_scanline >= maria_displayArea.top && maria_scanline <= maria_displayArea.bottom) {
    ...
    if(maria_scanline == maria_displayArea.top) {
      ...
      maria_WriteLineRAM(maria_surface + ((maria_scanline - maria_displayArea.top) * 320));
    }
